...
export default function PopTeachers() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setData(res.data.view.slice(0, 3));
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex flex-wrap justify-evenly items-center grid-cols-3 gap-4 px-8">
          {data.map((item) => (
            <motion.div
              // variants={reveal}
              key={item._id}
              className="relative w-[305px]  rounded-full  h-[535px] group"
            >
              <img
                src={item.prof[0].url}
                alt="Teacher"
